...
<template>
    <div class="subcontractor-employees">
        <div class="tab-header">
            ...
            <p>
                Give below total Number of permanent Construction employees at
                present on your payroll / Berikan dibawah ini Jumlah total
                karyawan Konstruksi tetap saat ini dalam daftar gaji Anda
            </p>
        </div>
                ...
                <table class="employees-table">
                    ...
                    <tbody>
                        <tr
                            ...
                            :class="{
                                'highlight-row':
                                    employee.classification === 'Others',
                            }"
                        >
                    ...
                    <tfoot>
                        <tr class="total-row">
                            ...
                            <td>
                                <strong>{{
                                    getTotalByType("craft_labor")
                                }}</strong>
                            </td>
                            <td>
                                <strong>{{ getTotalByType("foreman") }}</strong>
                            </td>
                            <td>
                                <strong>{{
                                    getTotalByType("supervisor")
                                }}</strong>
                            </td>
                            <td>
                                <strong>{{ getTotalByType("manager") }}</strong>
                            </td>
                            <td>
...
function resetAllData() {
    if (
        confirm(
            "Are you sure you want to reset all employee data? This action cannot be undone."
        )
    ) {
...
async function saveEmployees() {
    ...
    try {
        ...
        if (response.data.success) {
            ...
            showAlert(
                "error",
                response.data.message || "Failed to save employee data"
            );
        }
    } catch (error) {
