...
<template>
  <div class="vendor-documents">
    <div class="documents-header">
      ...
      <p>Upload dan kelola dokumen persyaratan vendor sesuai tipe perusahaan Anda</p>
    </div>
    ...
    <div class="document-requirements">
      <div class="requirement-header">
        <h3>
          <i :class="vendorTypeConfig.icon" :style="{ color: vendorTypeConfig.color }"></i>
          Dokumen untuk {{ vendorTypeConfig.name }}
        </h3>
        <div class="progress-indicator">
          <span class="uploaded-count">{{ uploadedCount }}</span>
          /
          <span class="total-count">{{ requiredDocuments.length }}</span>
          dokumen terupload
        </div>
      </div>
          ...
          <div class="document-header">
            <div class="doc-info">
              ...
              <div>
                <h4>{{ doc.name }}</h4>
                <p>{{ doc.description }}</p>
                <span v-if="doc.required" class="required-badge">Wajib</span>
                <span v-else class="optional-badge">Opsional</span>
              </div>
            </div>
            <div class="doc-status">
              <i v-if="doc.uploaded" class="fas fa-check-circle text-success"></i>
            ...
            <div v-else class="uploaded-file">
              <div class="file-info">
                ...
                <div>
                  ...
                  <small class="file-size">{{ doc.fileSize }} • {{ doc.uploadDate }}</small>
                </div>
              </div>
              <div class="file-actions">
                <button @click="downloadFile(doc)" class="action-btn download">
                  <i class="fas fa-download"></i>
                </button>
                <button @click="viewFile(doc)" class="action-btn view">
                  <i class="fas fa-eye"></i>
                </button>
                <button @click="deleteFile(doc)" class="action-btn delete">
      ...
      <div v-else class="history-list">
        ...
        >
          ...
          <div class="history-content">
            <p><strong>{{ history.action }}</strong> - {{ history.document_name }}</p>
            <small>{{ history.created_at }} oleh {{ history.user_name }}</small>
          </div>
        </div>
      </div>
    </div>
...
const vendorTypeConfig = computed(() => {
  const configs = {
    'SC': { name: 'Subcontractor', icon: 'fas fa-hard-hat', color: '#fd7e14' },
    'DS': { name: 'Distributor', icon: 'fas fa-truck', color: '#28a745' },
    'MF': { name: 'Manufacturer/Fabrication', icon: 'fas fa-industry', color: '#6f42c1' },
    'FW': { name: 'Forwarder', icon: 'fas fa-shipping-fast', color: '#20c997' }
  }
...
async function handleFileUpload(event, document) {
  ...
  if (file.size > document.maxSize * 1024 * 1024) {
    showAlert('error', `File terlalu besar. Maksimal ${document.maxSize}MB`)
    return
  }
  ...
  if (!allowedTypes.includes(fileExtension)) {
    showAlert('error', `Tipe file tidak diizinkan. Gunakan: ${document.accept}`)
    return
  }
  ...
  try {
    ...
    const response = await axios.post('/api/vendor/documents/upload', formData, {
      headers: {
    ...
    showAlert('error', error.response?.data?.message || 'Gagal upload dokumen')
  } finally {
...
async function downloadFile(document) {
  try {
    const token = localStorage.getItem('token')
    const response = await axios.get(`/api/vendor/documents/${document.id}/download`, {
  ...
  try {
    ...
    if (response.data.success) {
      ...
      requiredDocuments.value.forEach(doc => {
        const uploaded = uploadedDocs.find(up => up.document_type === doc.id)
        if (uploaded) {
          doc.uploaded = true
          doc.filename = uploaded.filename
          doc.fileSize = uploaded.file_size
          doc.uploadDate = new Date(uploaded.created_at).toLocaleDateString('id-ID')
        }
      })
    }
  } catch (error) {
