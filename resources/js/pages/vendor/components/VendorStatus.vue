      ...
      <div class="requirements-list">
        ...
        >
          ...
          <div class="requirement-content">
            ...
            <div v-if="requirement.deadline" class="requirement-deadline">
        ...
        <div class="detail-card">
          ...
          <div class="detail-content">
            <div class="detail-item">
              ...
              <span class="value">{{ vendor.completion_percentage }}%</span>
            ...
            <div class="detail-item">
              ...
              <span :class="['value', 'status', vendor.profile_completed ? 'complete' : 'incomplete']">
                {{ vendor.profile_completed ? 'Lengkap' : 'Belum Lengkap' }}
              </span>
            </div>
          </div>
        </div>
        ...
        <div class="detail-card">
          ...
          <div class="detail-content">
            <div class="detail-item">
              ...
              <span class="value">{{ documentStats.uploaded }}/{{ documentStats.total }}</span>
            </div>
            <div class="detail-item">
              <span class="label">Status Dokumen:</span>
              <span :class="['value', 'status', documentStats.status]">
        ...
        <div class="detail-card">
          ...
          <div class="detail-content">
            <div class="detail-item">
              ...
              <span :class="['value', 'status', user.email_verified_at ? 'verified' : 'unverified']">
                {{ user.email_verified_at ? 'Terverifikasi' : 'Belum Verified' }}
              </span>
            </div>
            <div class="detail-item">
...
const statusDescription = computed(() => {
  const descriptions = {
    'pending': 'Aplikasi vendor Anda sedang menunggu untuk direview oleh tim PLN.',
    'under_review': 'Tim PLN sedang melakukan review terhadap data dan dokumen yang Anda kirimkan.',
    'approved': 'Selamat! Vendor Anda telah disetujui dan dapat berpartisipasi dalam tender.',
    'rejected': 'Mohon maaf, aplikasi vendor Anda ditolak. Silakan perbaiki sesuai catatan.',
    'incomplete': 'Lengkapi data dan dokumen yang diperlukan untuk melanjutkan proses verifikasi.'
  }
  return descriptions[verificationData.value.status] || 'Status tidak diketahui.'
})
...
const timelineSteps = computed(() => {
  const baseSteps = [
    ...
    },
    {
      ...
      date: props.user.email_verified_at ? formatDate(props.user.email_verified_at) : null
    },
    {
      ...
      status: props.vendor.completion_percentage >= 80 ? 'completed' : 'in-progress',
      date: null
    },
    {
      id: 'document_upload',
      ...
      icon: 'fas fa-upload',
      status: documentStats.value.status === 'complete' ? 'completed' : 'pending',
      date: null
    },
    {
      id: 'verification',
      ...
      icon: 'fas fa-search',
      status: verificationData.value.status === 'approved' ? 'completed' : 
              verificationData.value.status === 'under_review' ? 'in-progress' : 'pending',
      date: verificationData.value.verified_at ? formatDate(verificationData.value.verified_at) : null,
...
const requirements = computed(() => {
  ...
  if (!props.user.email_verified_at) {
    reqs.push({
      id: 'email_verification',
      title: 'Verifikasi Email',
      description: 'Email Anda belum diverifikasi. Silakan cek kotak masuk.',
  ...
  if (props.vendor.completion_percentage < 80) {
    reqs.push({
      id: 'complete_profile',
      title: 'Lengkapi Profil',
      description: 'Profil perusahaan masih kurang lengkap. Minimal 80% harus terisi.',
...
.status-review { border-left-color: #007bff; }
.status-approved { border-left-color: #28a745; }
.status-rejected { border-left-color: #dc3545; }
.status-incomplete { border-left-color: #fd7e14; }
...
.status-pending .status-icon { background: #ffc107; }
.status-review .status-icon { background: #007bff; }
.status-approved .status-icon { background: #28a745; }
.status-rejected .status-icon { background: #dc3545; }
.status-incomplete .status-icon { background: #fd7e14; }
...
.status-review .badge { background: #cce7ff; color: #004085; }
.status-approved .badge { background: #d4edda; color: #155724; }
.status-rejected .badge { background: #f8d7da; color: #721c24; }
.status-incomplete .badge { background: #ffe8d1; color: #8b4513; }
