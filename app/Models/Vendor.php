...
class Vendor extends Model
{
    ...
    public function getCompletionPercentage()
    {
        ...
        // Existing vendor fields calculation...
        // (keep your existing code for general vendor fields)

        // Get field mappings for this vendor type
        $fieldMappings = VendorFieldMapping::where('tipe_perusahaan', $this->tipe_perusahaan)->get();
        ...
        foreach ($fieldMappings as $mapping) {
            ...
            if ($fieldType === 'json') {
                if (is_array($value) && !empty($value)) {
                    $filledFields++;
                } elseif (is_string($value) && !empty($value) && $value !== '[]') {
                    $filledFields++;
                }
            } else {
        ...
        // Add distributor specific fields calculation for DS type
        if ($this->tipe_perusahaan === 'DS' && $this->distributor) {
            $distributor = $this->distributor;

            $distributorFields = [
                'engineering_activities' => 'text',
                'priority_treatment' => 'enum',
                'complaint_agreement' => 'enum',
                'has_contact_centre' => 'enum',
            ];

            foreach ($distributorFields as $fieldName => $fieldType) {
                $value = $distributor->{$fieldName};
                $totalFields++;
